...
class ApimNamedValueScenarioTest(ScenarioTest):
    ...
    def _update_named_value(self):
        ...
        self.cmd('apim named-value update -g "{rg}" -n "{apim}" --value-id "{nv_id}" --value "{updated_value}"')
    ...
    def _create_named_value(self):
        ...
        self.cmd('apim named-value create -g {rg} -n {apim} --display-name "{display_name}" --value "{value}" --value-id "{nv_id}" --secret false --tags "{tags}"', checks=[
    ...
    def _create_named_value_secret(self):
        ...
        self.cmd('apim named-value create -g "{rg}" --service-name "{apim}" --display-name "{secret_display_name}" --value "{secret_value}" --value-id "{nv_secret_id}" --secret true --tags "{secret_tags}"', checks=[
    ...
    def _then_get_named_value_secret(self):
        self.cmd('apim named-value show -g "{rg}" -n "{apim}" --value-id {secret_nv_id} --secret true', checks=[
